...
int MstState::AugmentCheckerMove(CheckerMove* cmove, int player,
                                        int start) const {
  int end = cmove->num;
  if (end != kPassPos) {
...
void MstState::InformationStateAsNormalizedVector(
...
MstState::MstState(std::shared_ptr<const Game> game,
...
int MstState::board(int player, int pos) const {
  if (pos == kBarPos) {
    return bar_[player];
  } else {
...
Player MstState::CurrentPlayer() const {
  return IsTerminal() ? kTerminalPlayerId : Player{cur_player_};
}

int MstState::Opponent(int player) const { return 1 - player; }

void MstState::RollDice(int outcome) {
  dice_.push_back(kChanceOutcomeValues[outcome][0]);
  dice_.push_back(kChanceOutcomeValues[outcome][1]);
}

int MstState::DiceValue(int i) const {
  SPIEL_CHECK_GE(i, 0);
...
void MstState::UndoAction(int player, Action action) {
  {
...
Action MstState::TranslateAction(int from1, int from2,
...
Action MstState::EncodedBarMove() const { return 24; }

Action MstState::EncodedPassMove() const { return 25; }

Action MstState::CheckerMovesToSpielMove(
...
std::vector<CheckerMove> MstState::SpielMoveToCheckerMoves(
...
bool MstState::IsPosInHome(int player, int pos) const {
  switch (player) {
    case kXPlayerId:
...
int MstState::CheckersInHome(int player) const {
  int c = 0;
  for (int i = 0; i < 6; i++) {
...
bool MstState::AllInHome(int player) const {
  if (bar_[player] > 0) {
    return false;
  }
...
int MstState::HighestUsableDiceOutcome() const {
...
int MstState::FurthestCheckerInHome(int player) const {
...
bool MstState::UsableDiceOutcome(int outcome) const {
  return (outcome >= 1 && outcome <= 6);
}

int MstState::PositionFromBar(int player, int spaces) const {
  if (player == kXPlayerId) {
    return -1 + spaces;
  } else if (player == kOPlayerId) {
    return 24 - spaces;
...
int MstState::PositionFrom(int player, int pos, int spaces) const {
  if (pos == kBarPos) {
...
int MstState::NumOppCheckers(int player, int pos) const {
  return board_[Opponent(player)][pos];
}

int MstState::GetDistance(int player, int from, int to) const {
...
bool MstState::IsOff(int player, int pos) const {
  // Returns if an absolute position is off the board.
  return ((player == kXPlayerId && pos > 23) ||
          (player == kOPlayerId && pos < 0));
}

bool MstState::IsFurther(int player, int pos1, int pos2) const {
  if (pos1 == pos2) {
    return false;
  }
...
int MstState::GetToPos(int player, int from_pos, int pips) const {
  if (player == kXPlayerId) {
...
bool MstState::IsLegalFromTo(int player, int from_pos, int to_pos,
...
std::string MstState::DiceToString(int outcome) const {
  if (outcome > 6) {
...
int MstState::CountTotalCheckers(int player) const {
  int total = 0;
  for (int i = 0; i < 24; ++i) {
...
int MstState::IsGammoned(int player) const {
  // Does the player not have any checkers borne off?
  return scores_[player] == 0;
}

int MstState::IsBackgammoned(int player) const {
...
std::set<CheckerMove> MstState::LegalCheckerMoves(int player) const {
...
bool MstState::ApplyCheckerMove(int player, const CheckerMove& move) {
  // Pass does nothing.
  if (move.pos < 0) {
    return false;
  }
...
void MstState::UndoCheckerMove(int player, const CheckerMove& move) {
...
std::vector<Action> MstState::ProcessLegalMoves(
...
std::vector<Action> MstState::LegalActions() const {
  ...
  MstState* state = dynamic_cast<MstState*>(cstate.get());
  std::set<std::vector<CheckerMove>> movelist;
...
std::vector<std::pair<Action, double>> MstState::ChanceOutcomes() const {
...
void MstState::SetState(int cur_player, bool double_turn,
...
double MstGame::MaxUtility() const {
  switch (scoring_type_) {
    case ScoringType::kWinLossScoring:
      return 1;
    case ScoringType::kEnableGammons:
